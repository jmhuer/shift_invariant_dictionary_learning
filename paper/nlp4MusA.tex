...
\textbf{Groove Model} 
\\
For the Groove, drum dataset our design is: 
\\ 
\centerline{\emph{k-WTA - 1D-Conv [100, 1] } }\\
We implement a decaying k-WTA since this empirically improved our model accuracy. We begin with $k=10$ and decay to $k=1$ over $1000$ Epochs. The Convolutions are similarly also non-overlapping strides. We implemented the model in Pytorch, with MSE loss on reconstruction, AdamW optimizer. We do not batch, we train on the full dataset. We  preprocess the dataset to match 120 bpm, 4/4 time signature and ony train on 1 bar of music.  For this dataset we have good understanding of repeating time intervals and structure, unlike maestro dataset. 

\subsection{De-noising musical ideas}
\label{ssec:first}

 \textbf{Keep Top N Dictionary words}
